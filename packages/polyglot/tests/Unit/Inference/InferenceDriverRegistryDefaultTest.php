...
it('returns the same bundled registry instance on every call', function () {
    ...
    expect(InferenceDriverRegistry::default())->toBe(InferenceDriverRegistry::default());
...
it('does not let a derived registry affect the shared one', function () {
    ...
    $derived = InferenceDriverRegistry::default()
    ...
    expect($derived->has('openai'))->toBeFalse()
        ...
        ->and(InferenceDriverRegistry::default()->has('openai'))->toBeTrue()
        ->and(InferenceDriverRegistry::default()->has('custom-x'))->toBeFalse();
})->group('driver-registry');
...
it('resolves every bundled driver name to a declarative driver', function () {
    ...
    foreach ($names as $name) {
        ...
        expect($driver)
            ...
            ->toBeInstanceOf(BaseInferenceRequestDriver::class);
    }
})->group('driver-registry');
...
it('keeps the four aliases that share one OpenAI-compatible behaviour', function () {
    ...
    // InferenceDriverSpec now. The requests they emit are pinned byte-for-byte in
    // DriverGoldenRequestTest's equivalence classes.
    $registry = InferenceDriverRegistry::default();
    $httpClient = (new HttpClientBuilder())->create();
    $events = new EventDispatcher();
    ...
    foreach (['moonshot', 'ollama', 'openai-compatible', 'together'] as $alias) {
        ...
        expect($driver)->toBeInstanceOf(BaseInferenceRequestDriver::class);
    }
})->group('driver-registry');
