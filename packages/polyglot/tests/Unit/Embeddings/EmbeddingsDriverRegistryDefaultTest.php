...
it('returns the same bundled registry instance on every call', function () {
    ...
    expect(EmbeddingsDriverRegistry::default())->toBe(EmbeddingsDriverRegistry::default());
...
it('does not let a derived registry affect the shared one', function () {
    ...
    $derived = EmbeddingsDriverRegistry::default()
    ...
    expect($derived->has('openai'))->toBeFalse()
        ...
        ->and(EmbeddingsDriverRegistry::default()->has('openai'))->toBeTrue()
        ->and(EmbeddingsDriverRegistry::default()->has('custom-x'))->toBeFalse();
})->group('driver-registry');
