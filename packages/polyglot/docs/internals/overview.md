...
Inference and embeddings are separate subsystems with parallel class hierarchies, but a few primitives belong to neither. They live under `Cognesy\Polyglot\Support\`:
...
- **`Support\Retry\RetryBackoff`, `RetryJitter`, `RetryPolicyInvariants`** -- the delay computation, jitter strategies and constructor invariants shared by `InferenceRetryPolicy` and `EmbeddingsRetryPolicy`.
...
The retry and redaction classes previously lived under `Inference\`, which forced every embeddings file that needed them to import from the inference subsystem. `Cost` was already neutral, at a top-level `Polyglot\Pricing\`; it moved here so that `Support\` is the package's single answer to "where do things neither subsystem owns live?" rather than one of two competing conventions.

The old fully-qualified names all still resolve, through a lazily-registered alias in `src/deprecated_aliases.php`, and will be removed in the next major version.
