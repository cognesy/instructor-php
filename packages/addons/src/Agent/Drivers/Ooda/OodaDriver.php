...
namespace Cognesy\Addons\Agent\Drivers\Ooda;
...
use Cognesy\Addons\Agent\Drivers\Ooda\Actions\MakeOodaPrompt;
use Cognesy\Addons\Agent\Drivers\Ooda\Data\OodaDecision;
use Cognesy\Addons\Agent\Drivers\Ooda\Utils\OodaFormatter;
...
/**
 * OODA Loop Driver for structured agent reasoning.
 *
 * Implements the Observe-Orient-Decide-Act decision cycle:
 ...
 *
 * This driver produces explicit, traceable reasoning at each step,
 * making the agent's decision-making process transparent.
 */
final class OodaDriver implements CanUseTools
{
    ...
    public function useTools(AgentState $state, Tools $tools, CanExecuteToolCalls $executor): AgentStep {
        ...
        $decision = $extraction->unwrap()['decision'];
        $response = $extraction->unwrap()['response'];

        if ($this->verbose) {
            $this->logOodaDecision($decision);
        }
        ...
        // Unwrap args if LLM wrapped them in an array: [{"path":"..."}] -> {"path":"..."}
        $this->unwrapDecisionArgs($decision);
        ...
        if (!$decision->isCall()) {
            return $this->buildFinalAnswerStep($decision, $usage, $response, $messages);
        }
    ...
    private function buildSystemPrompt(Tools $tools): string {
        return (new MakeOodaPrompt($tools))();
    }

    private function extractDecision(Messages $messages, string $system): array {
        $pending = $this->extractDecisionWithUsage($messages, $system, OodaDecision::class);
        /** @var OodaDecision $decision */
    ...
    private function makeFollowUps(OodaDecision $decision, ToolExecutions $executions): Messages {
        $formatter = new OodaFormatter();
        $messages = Messages::empty()->appendMessage($formatter->assistantOodaMessage($decision));
    ...
    private function unwrapDecisionArgs(OodaDecision $decision): void {
        $args = $decision->args;
        // Handle [{"key": "value"}] -> {"key": "value"}
        if (count($args) === 1 && isset($args[0]) && is_array($args[0])) {
            $decision->setArgs($args[0]);
        }
    }

    private function logOodaDecision(OodaDecision $decision): void {
        fwrite(STDERR, "\n  [OODA] " . str_repeat("─", 60) . "\n");
        fwrite(STDERR, "  [OBSERVE] Goal: {$decision->goal}\n");
        fwrite(STDERR, "  [OBSERVE] State: {$decision->currentState}\n");
        fwrite(STDERR, "  [OBSERVE] Progress: {$decision->progress}%\n");
        if (!empty($decision->obstacles)) {
            fwrite(STDERR, "  [OBSERVE] Obstacles: {$decision->obstacles}\n");
        }
        fwrite(STDERR, "  [ORIENT] Analysis: {$decision->analysis}\n");
        if (!empty($decision->options)) {
            fwrite(STDERR, "  [ORIENT] Options: " . implode(', ', $decision->options) . "\n");
        }
        fwrite(STDERR, "  [ORIENT] Reasoning: {$decision->reasoning}\n");
        fwrite(STDERR, "  [DECIDE] Type: {$decision->type} (confidence: {$decision->confidence}%)\n");
        if ($decision->isCall()) {
            $args = json_encode($decision->args, JSON_UNESCAPED_SLASHES);
            fwrite(STDERR, "  [ACT] Tool: {$decision->tool}({$args})\n");
        } else {
            $preview = substr($decision->answer(), 0, 60);
            fwrite(STDERR, "  [ACT] Answer: {$preview}...\n");
        }
        fwrite(STDERR, "  " . str_repeat("─", 64) . "\n");
    }
}
