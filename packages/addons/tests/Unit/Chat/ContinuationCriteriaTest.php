...
final class CountingChatCriterion implements CanEvaluateContinuation
{
...
it('stops when steps limit reached', function () {
    ...
    expect($crit->evaluate($state)->decision)->toBe(ContinuationDecision::AllowContinuation);
    // simulate two steps
...
it('chat continuation criteria mutates when adding', function () {
    ...
    expect($criteria->evaluateAll(new ChatState())->decision())->toBe(ContinuationDecision::AllowStop);
...
it('chat continuation criteria uses priority resolution', function () {
    ...
    // ForbidContinuation wins → ForbidContinuation (guard denied, shouldContinue returns false)
    expect($criteria->evaluateAll(new ChatState())->decision())->toBe(ContinuationDecision::ForbidContinuation);
