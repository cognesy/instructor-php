...
## SecretResolver — application-owned layered interpolation
...
$secrets = new SecretResolver(
    ...
    DotenvFileSecretSource::optional('/workspace/.env', 'workspace-env'),
    DotenvFileSecretSource::optional('/app/credentials.env', 'app-credentials'),
);

$config = Config::fromPaths('/app/config')
    ->withTemplate(new EnvTemplate($secrets));

$resolved = $secrets->resolve('OPENAI_API_KEY');
$resolved?->source;    // safe provenance, such as "workspace-env"
$resolved?->toArray(); // name, configured, and source; never the value
```

Sources are queried in constructor order and the first non-empty value wins.
Use `ResolvedSecret::value()` only at the configuration-consumption boundary.
The application, not this package, owns credential persistence and permissions.
