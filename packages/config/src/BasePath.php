...
class BasePath
{
    ...
    /**
     * Resolve a single path relative to the application base path.
     *
     * @param string $path Optional path to append to the base path
     * @return string
     * @throws \Exception When unable to determine the base path
    ...
    /**
     * Resolve all existing paths from candidates. Missing paths are ignored.
     *
     * @return array<int, string>
     */
    public static function all(string ...$paths): array {
        return self::resolveExisting(...$paths);
    }

    /**
     * Resolve existing paths from candidates. Missing paths are ignored.
     *
     * @return array<int, string>
     */
    public static function resolveExisting(string ...$paths): array {
        $resolved = [];
        ...
        foreach ($paths as $path) {
            ...
            try {
                $candidate = self::resolve($trimmed);
            } catch (Throwable) {
                continue;
            }

            if (!self::pathExists($candidate)) {
                continue;
            }
    ...
    /**
     * Resolve first existing path from candidates. Throws if none can be resolved.
     *
     * @throws InvalidArgumentException
     */
    public static function resolveFirst(string ...$paths): string {
        $resolved = self::resolveExisting(...$paths);
        if ($resolved !== []) {
            return $resolved[0];
        }

        throw new InvalidArgumentException('Unable to resolve any existing path from provided candidates');
    }
     ...
     * @return void
     */
    public static function setDetectionMethods(array $methods): void {
        $instance = self::getInstance();
        $instance->detectionMethods = array_values(array_filter(
            $methods,
