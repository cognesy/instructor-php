...
This page covers pinning exact, scripted behavior against a fake driver. When you need to grade an agent's behavior against a real driver -- including cases where "acceptable" is a matter of degree that only a model can judge -- see [Agent Evals](21-evals).
