...
- **[Evals](21-evals)** -- the case API, targets, and datasets that `agents-eval` discovers and runs.
- **[Eval traces and artifacts](24-eval-traces-and-artifacts)** -- what a case's console line, `judges/NNN.json`, and `provenance` block mean in full, including the `guardsWarningObserved` caveat referenced above.
