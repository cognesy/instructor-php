...
use Cognesy\Agents\Agent\Continuation\Criteria\ResponseContentCheck;
...
it('calls predicate with correct Messages type', function () {
    ...
    $responseResolver = fn(AgentState $state): ?Messages => $state->messages();
    $predicateCalls = [];
    ...
    $state = AgentState::empty()->withMessages($testMessages);
...
it('returns AllowContinuation when response resolver returns null (permits bootstrap)', function () {
    $responseResolver = fn(AgentState $state): ?Messages => null;
...
it('correctly evaluates non-empty content predicate', function () {
    ...
    $result1 = $check->evaluate(AgentState::empty()->withMessages($nonEmptyMessages));
    expect($result1->decision)->toBe(ContinuationDecision::RequestContinuation);

    // Test with empty content: predicate false → AllowStop
    $result2 = $check->evaluate(AgentState::empty()->withMessages($emptyMessages));
...
it('works with multiple messages and evaluates last message correctly', function () {
    ...
    $check = new ResponseContentCheck($responseResolver, $predicate);
    $state = AgentState::empty()->withMessages($multipleMessages);
...
it('can be used with different predicate logic', function () {
    ...
    $state = AgentState::empty()->withMessages($messages);
    ...
    $shortResult = $check->evaluate(AgentState::empty()->withMessages($shortMessages));
...
it('handles Messages collection properly when checking for continuation patterns', function () {
    ...
    $result1 = $check->evaluate(AgentState::empty()->withMessages($validMessages));
    expect($result1->decision)->toBe(ContinuationDecision::RequestContinuation);
    ...
    $result3 = $check->evaluate(AgentState::empty()->withMessages($whitespaceMessages));
