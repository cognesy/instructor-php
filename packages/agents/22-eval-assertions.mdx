...
These read the accumulated `AgentRun` -- see [Agent Evals](21-evals) for how `send()` builds it across turns. None of them record a message on failure; a failing outcome assertion is identified by its assertion name alone (`succeeded`, `stopped`, `messageIncludes`, `outputEquals`, `outputMatches`).
