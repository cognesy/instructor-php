...
The agent loop emits ten trigger types at well-defined points during execution. Each trigger corresponds to a specific moment in the loop's lifecycle, and understanding when each fires is essential for placing your hooks correctly:
...
| `BeforeInferenceRequest` | After the request is built, before provider execution | Agent state, `InferenceRequest` |
| `AfterInferenceResponse` | After provider execution, before step archival | Agent state, `InferenceRequest`, `InferenceResponse` |
...
HookTrigger::BeforeInferenceRequest; // 'before_inference_request'
HookTrigger::AfterInferenceResponse; // 'after_inference_response'
...
BeforeExecution
  ...
  +---> BeforeStep
  |       |
  |       +---> BeforeInferenceRequest ---> [LLM Call] ---> AfterInferenceResponse
  |       |
...
| `inferenceRequest()` | `?InferenceRequest` | The concrete request about to cross the provider boundary | `BeforeInferenceRequest`, `AfterInferenceResponse` |
| `inferenceResponse()` | `?InferenceResponse` | The provider response before it is archived into the step | `AfterInferenceResponse` |
...
$ctx = HookContext::beforeInferenceRequest($state, $inferenceRequest);
$ctx = HookContext::afterInferenceResponse($state, $inferenceRequest, $inferenceResponse);
...
HookTriggers::beforeInferenceRequest(); // Just BeforeInferenceRequest
HookTriggers::afterInferenceResponse(); // Just AfterInferenceResponse
...
Use `withMetadataEntry($key, $value)` to attach one value to the hook context's own metadata channel without replacing the whole metadata array. `withMetadata(array $metadata)` remains available when the complete array is already known.
...
<a name="inference-hooks"></a>
## Intercepting Inference

`BeforeInferenceRequest` is the last mutation point before `ToolCallingDriver` creates the pending provider operation. Replacing the request here changes what the inference runtime actually receives:

```php
$hook = new CallableHook(function (HookContext $ctx): HookContext {
    $request = $ctx->inferenceRequest();
    if ($request === null) {
        return $ctx;
    }

    return $ctx->withInferenceRequest($request->withModel('fallback-model'));
});
```

The request's messages have already been compiled before this hook runs. A state
replacement is carried forward to events, response interception, tool execution,
and the resulting step, but it does not retroactively recompile the current
request. To change the current provider call, replace `inferenceRequest`; use a
state change for later lifecycle phases or subsequent steps.

`AfterInferenceResponse` runs after the provider returns but before events, tool execution, and `AgentStep` archival consume the response. The context also retains the request for correlation:
...
These hooks currently describe the inference boundary owned by `ToolCallingDriver`. `ReActDriver`, inference performed inside capabilities, and custom drivers do not emit them unless they explicitly implement `CanAcceptLifecycleInterceptor` and perform the interception themselves. Drivers without that interface continue to execute unchanged.

`InferenceRequestStarted` and `InferenceResponseReceived` are dispatched after their corresponding hook, so event observers see the model, usage, and finish reason that the runtime and step actually use. Both events retain the same pending-inference execution ID.

<a name="mutation-contracts"></a>
## Mutation Contracts

Each `HookTrigger` declares its intended mutable fields through `mutableFields()`. Contract checking is opt-in in 2.6, so existing third-party hooks keep their current behavior by default.

| Trigger | Intended mutable fields |
|---------|-------------------------|
| `BeforeExecution`, `BeforeStep`, `AfterStep`, `OnStop`, `AfterExecution` | `state`, `metadata` |
| `BeforeInferenceRequest` | `state`, `inferenceRequest`, `metadata` |
| `AfterInferenceResponse` | `state`, `inferenceResponse`, `metadata` |
| `BeforeToolUse` | `state`, `toolCall`, blocking fields, `metadata` |
| `AfterToolUse` | `state`, `toolExecution`, `metadata` |
| `OnError` | `state`, `errorList`, `metadata` |

Diagnostic mode emits one `HookContractViolated` event per disallowed field and deliberately preserves the returned context:

```php
$stack = (new HookStack(new RegisteredHooks(), $events))
    ->with($hook, HookTriggers::beforeStep(), name: 'my_hook')
    ->withContractDiagnostics();
```

Strict mode emits the diagnostic and then throws `HookContractViolationException` at the first disallowed mutation. It is useful in tests and controlled deployments:

```php
$stack = $stack->strict();
```

When the stack is assembled through `AgentBuilder`, enable the same policies
directly on the builder:

```php
$diagnosticAgent = AgentBuilder::base()
    ->withHookContractDiagnostics()
    ->withCapability($capability)
    ->build();

$strictAgent = AgentBuilder::base()
    ->withStrictHookContracts()
    ->withCapability($capability)
    ->build();
```

The contract narrows which top-level `HookContext` fields a trigger should replace. It does not constrain changes inside an allowed `AgentState`; hooks still own the semantic correctness of state transformations.

<a name="blocking-tools"></a>
## Blocking Tool Execution

In a `BeforeToolUse` hook, you can prevent a tool from executing by calling `blockToolExecution()` on the context. The existing `withToolExecutionBlocked()` method remains a compatibility alias. This is a powerful safety mechanism for restricting which tools the model can invoke at runtime:
...
$context->blockToolExecution('This tool requires admin privileges.');

// With default message (includes HookContext details)
$context->blockToolExecution();
```
