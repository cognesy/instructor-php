...
5. **Resolved profile and binding** -- the profile is snapshotted from the
   resolved, unbound tools, driver, capabilities, and hooks. The completed profile
   is then injected into profile-aware tools, drivers, and hooks.
...
### Resolved profiles and derived prompts

`AgentProfile` is the deterministic description of the loop that was actually
built. `UseSystemPrompt` installs a lifecycle hook that derives its tool list and
guidelines from this resolved profile, so adding or removing a tool changes the
next composed prompt without maintaining a second hardcoded list:

```php
use Cognesy\Agents\Capability\Prompt\UseSystemPrompt;

$agent = AgentBuilder::base()
    ->withCapability(new UseTools($searchTool))
...
Profile creation deliberately happens before profile binding to avoid a circular
dependency. A `CanAcceptAgentProfile` implementation may use the injected profile
for runtime behavior, but its descriptor and profile contribution methods must
not depend on that injected value: those methods were already read to create the
snapshot. Loop `with*()` operations recompute the profile and rebind consumers.

Tools produced by deferred providers are marked with `deferred: true` in the
profile and in `describe()` output, including a deferred tool that replaces an
eager tool with the same name.
...
### Discovering capabilities from Composer manifests

Trusted packages can publish zero-argument capability and tool classes under
`extra.cognesy-agents` in `composer.json`. Discovery is explicit and populates the
same registries used by definitions:

```php
$result = CapabilityDiscovery::discover($capabilities, $tools);
```

Call discovery during application bootstrap before a `DefinitionLoopFactory`
resolves definition references. Root-package declarations override vendor
declarations; malformed entries are reported by `$result->errors()`. See
[Agent Templates](14-agent-templates.md#opt-in-composer-manifest-discovery) for
the manifest shape and trust boundary.

### Hook contract diagnostics

`AgentBuilder::withHookContractDiagnostics()` emits contract-violation events
while preserving hook output. `withStrictHookContracts()` turns the same checks
into exceptions and is intended for tests and controlled deployments. Both modes
are opt-in; default hook execution remains compatible and permissive.
