...
## Describing the Resolved Agent

Use `describe()` when a host application, CLI, test, or operator needs a deterministic description of the agent that was actually built:
...
The description includes the resolved identity, driver, safe LLM configuration, installed capabilities, resolved tools, registered hooks, self-knowledge summary, and public metadata. Tool entries use the same resolved names and prompt-visibility rule as prompt composition. Hook entries are ordered by lifecycle trigger, then priority, then registration order.

`toArray()` is the machine-readable contract. Its field order and collection ordering are deterministic within a major release; minor releases may add fields. Consumers should read fields by name and tolerate additions.

The LLM section is intentionally a safe projection. It may include the driver, model, and token limits, but never exposes API keys, authorization headers, credential-bearing URLs, provider options, or arbitrary connection metadata.

The tool list describes tools installed on the resolved loop. It is not a promise that every tool call will be authorized in every state: guards and policy hooks can still block a tool at execution time.

## Letting the Agent Describe Itself

Self-description is opt-in. Add `UseSelfDescription` to install the `describe_self` tool:
...
The tool receives the final resolved profile after all capabilities, tools, hooks, and driver configuration have been applied. Its full response is therefore the same safe array returned by `$agent->describe()->toArray()`, including the `describe_self` tool itself.

An agent can request the full description or narrow the result to `tools`, `capabilities`, or `hooks`. Narrowing returns one top-level field with the same ordering and values as the full description; it does not create a second discovery path.
