...
describe('ProcessingState Monadic Operations', function () {
    ...
    describe('map()', function () {
        ...
        it('short-circuits on failure', function () {
            ...
            $result = TransformState::with($state)
    ...
    describe('map()', function () {
        it('applies function returning ProcessingState', function () {
            $state = ProcessingState::with(10);
            $result = TransformState::with($state)->map(fn($x) => ProcessingState::with($x * 2));
        ...
        it('merges tags from both states', function () {
            ...
            $result = TransformState::with($state)->map(fn($x) => ProcessingState::with($x * 2, [$tag2]));
        ...
        it('short-circuits on original failure', function () {
            ...
            $output = TransformState::with($state)->map(function($x) use (&$executed) {
                $executed = true;
        ...
        it('propagates failure from mapped function', function () {
            $state = ProcessingState::with(10);
            $result = TransformState::with($state)->map(fn($x) =>
    ...
    describe('filter()', function () {
        it('passes when predicate returns true', function () {
            $state = ProcessingState::with(10);
            $result = TransformState::with($state)->failWhen(fn($x) => $x > 5);
        ...
        it('fails when predicate returns false', function () {
            $state = ProcessingState::with(10);
            $result = TransformState::with($state)->failWhen(fn($x) => $x > 15);
        ...
        it('uses custom error message', function () {
            $state = ProcessingState::with(10);
            $result = TransformState::with($state)->failWhen(fn($x) => $x > 15, 'Value too small');
        ...
        it('short-circuits on failure', function () {
            ...
            $result = TransformState::with($state)->failWhen(function($x) use (&$executed) {
                $executed = true;
                return true;
            });
    ...
    describe('monadic composition', function () {
        it('chains multiple operations', function () {
            $state = TransformState::with(ProcessingState::with(10))
                ->map(fn($x) => $x * 2)           // 20
        ...
        it('preserves and accumulates tags through chain', function () {
            ...
            $state = TransformState::with(ProcessingState::with(10, [$tag1]))
                ->map(fn($x) => $x * 2)
