...
/**
 * Tests verifying critical sequence behavior for LEGACY response iterator:
...
if (!class_exists('LegacyPerson')) {
    eval('class LegacyPerson { public string $name; public int $age; }');
}

it('[legacy] partials() yields multiple updates per sequence item as chunks arrive', function () {
    ...
    $pending = (new StructuredOutput())
        ...
        ->with(
            messages: 'test',
            responseModel: Sequence::of('LegacyPerson'),
...
})->group('sequence', 'streaming', 'critical', 'legacy');

it('[legacy] sequence() yields single update per COMPLETED sequence item only', function () {
...
it('[legacy] sequence() does not yield incomplete items even if many chunks arrive', function () {
...
it('[legacy] partials() yields MORE updates than sequence() for same stream', function () {
