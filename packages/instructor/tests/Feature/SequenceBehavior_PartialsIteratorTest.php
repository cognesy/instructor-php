...
/**
 * Tests verifying critical sequence behavior for PARTIALS response iterator:
...
if (!class_exists('PartialsPerson')) {
    eval('class PartialsPerson { public string $name; public int $age; }');
}

it('[partials] partials() yields multiple updates per sequence item as chunks arrive', function () {
    ...
    $pending = (new StructuredOutput())
        ...
        ->with(
            messages: 'test',
            responseModel: Sequence::of('PartialsPerson'),
...
})->group('sequence', 'streaming', 'critical', 'partials');

it('[partials] sequence() yields single update per COMPLETED sequence item only', function () {
...
it('[partials] sequence() does not yield incomplete items even if many chunks arrive', function () {
...
it('[partials] partials() yields MORE updates than sequence() for same stream', function () {
