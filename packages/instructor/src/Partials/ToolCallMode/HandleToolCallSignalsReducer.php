...
use Cognesy\Instructor\Streaming\PartialGen\ToolCallStreamState;
...
class HandleToolCallSignalsReducer implements Reducer
{
    ...
    public function init(): mixed {
        $this->state = ToolCallStreamState::empty(
            onStart: function (\Cognesy\Polyglot\Inference\Data\ToolCall $tc): void {
                $this->events->dispatch(new StreamedToolCallStarted(['toolCall' => $tc->toArray()]));
            },
            onUpdate: function (\Cognesy\Polyglot\Inference\Data\ToolCall $tc): void {
                $this->events->dispatch(new StreamedToolCallUpdated(['toolCall' => $tc->toArray()]));
            },
            onComplete: function (\Cognesy\Polyglot\Inference\Data\ToolCall $tc): void {
