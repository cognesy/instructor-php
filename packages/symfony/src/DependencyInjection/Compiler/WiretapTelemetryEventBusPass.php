...
final class WiretapTelemetryEventBusPass implements CompilerPassInterface
{
    public function process(ContainerBuilder $container): void
    {
        if (! $container->hasDefinition('instructor.telemetry.observation_bridge')) {
            return;
        }
        ...
        if ($this->hasMethodCall($definition, 'wiretap', 'instructor.telemetry.observation_bridge')) {
            return;
        }

        $definition->addMethodCall('wiretap', [new Reference('instructor.telemetry.observation_bridge')]);
    }
