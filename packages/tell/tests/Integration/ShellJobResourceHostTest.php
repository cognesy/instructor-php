...
it('denies by default before allocating a visible job identity', function (): void {
    $host = TellResourceHost::shellJobs(tellTestProject())->boot();
...
it('cancels idempotently and invalidates retained manager handles on disposal', function (): void {
    ...
    expect($first->state)->toBe(TellShellJobState::Cancelled)
        ...
        ->and(fn () => $jobs->all())->toThrow(TellResourceHostDisposedException::class)
        ->and(fn () => $host->health())->toThrow(TellResourceHostDisposedException::class);
});

it('keeps independent hosts and their job catalogues isolated', function (): void {
    $project = tellTestProject();
    $firstHost = TellResourceHost::shellJobs($project, approval: TellShellJobApprovals::allowAll())->boot();
    $secondHost = TellResourceHost::shellJobs($project, approval: TellShellJobApprovals::allowAll())->boot();
...
it('emits normalized redacted resource events without exposing commands or output', function (): void {
    ...
    try {
        ...
        expect($events->count())->toBeGreaterThan(3)
            ->and(array_unique(array_column($events->getArrayCopy(), 'schema')))->toBe(['tell.resource.event.v1'])
...
it('contains observer failures instead of corrupting resource lifecycle', function (): void {
    $host = TellResourceHost::shellJobs(
        tellTestProject(),
        approval: TellShellJobApprovals::allowAll(),
        observer: TellResourceObservers::callback(static fn () => throw new RuntimeException('observer failed')),
    )->boot();
...
it('boots a fresh isolated runtime each time a reusable builder is booted', function (): void {
    $builder = TellResourceHost::shellJobs(
        tellTestProject(),
        approval: TellShellJobApprovals::allowAll(),
    );
    $first = $builder->boot();
...
it('cancels multiple running jobs in reverse ownership order on shutdown', function (): void {
    ...
    expect($firstMarker)->not->toBeFile()
        ...
        ->and(array_column($cancelled, 'resourceId'))->toBe([$second->id, $first->id]);
});
