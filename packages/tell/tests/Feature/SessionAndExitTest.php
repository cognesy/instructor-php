...
it('continues a named session with prior messages in the next compiled request', function (): void {
    ...
    (new CommandTester($command))->execute(['prompt' => 'first turn', '--session' => 's1']);
    (new CommandTester($command))->execute(['prompt' => 'second turn', '--session' => 's1']);

    $session = $factory->sessions()->getSession(SessionId::from('s1'));
