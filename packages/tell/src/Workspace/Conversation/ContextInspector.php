 ...
 */
final readonly class ContextInspector
{
    ...
    public function inspect(
        ConversationInspection $conversation,
    ...
    /** @return list<string> */
    private function compactedFrom(History $history): array {
        $hashes = [];
    ...
    private function toolCallCount(History $history): int {
        return array_sum(array_map(
            static fn (HistoryTurn $entry): int => count($entry->turn->toolCalls()),
            $history->turns,
        ));
    }

    private function toolResultCount(History $history): int {
        return array_sum(array_map(
            static fn (HistoryTurn $entry): int => count($entry->turn->toolResults()),
            $history->turns,
        ));
    }
