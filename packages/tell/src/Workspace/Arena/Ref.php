...
final readonly class Ref
{
    ...
    public static function fromBytes(string $bytes): self {
        ...
        if ($data['head'] !== null && !is_string($data['head'])) {
            throw new ArenaIntegrityException('Tell ref head must be an Arena object hash or null.');
        }
        try {
            $ref = new self(
                $data['head'] === null ? null : new ObjectHash($data['head']),
    ...
    private static function provenanceFromData(mixed $data): ?Provenance {
        if ($data === null) {
            return null;
        }
        if (!is_array($data) || array_is_list($data) || !in_array(array_keys($data), [
        ...
        if ($data['branch'] !== null && $data['branch'] !== 'main') {
            self::assertBranchName($data['branch']);
        }
    ...
    private static function assertBranchName(string $name): void {
        if (preg_match('/\A[a-z][a-z0-9-]{0,63}\z/', $name) !== 1) {
            throw new ArenaIntegrityException('Tell ref provenance branch is invalid.');
        }
    }
}
