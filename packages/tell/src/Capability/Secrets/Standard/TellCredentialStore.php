...
final readonly class TellCredentialStore implements CanManageTellCredentials
{
    ...
    public function status(string $variable, string $directory): ?TellCredentialStatus {
        $resolved = (new StandardTellSecretResolver($this->paths, $directory))->resolve($variable);

        return match ($resolved) {
            null => null,
            default => new TellCredentialStatus($variable, $resolved->source),
        };
    }
