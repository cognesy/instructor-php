...
final readonly class TellResourceHostBuilder
{
    public function __construct(
        private string $projectDirectory,
        private TellShellJobPolicy $policy,
        private CanApproveTellShellJobs $approval,
        private CanObserveTellResources $observer,
    ) {}

    public function boot(): TellResourceHost
    {
        ...
        if (! $manager instanceof CordisTellShellJobs || $fiber->state() !== FiberState::Active) {
            ...
            throw new RuntimeException('The Tell resource host could not be booted.', previous: $error);
        }

        return new TellResourceHost($runtime, $manager, $unsubscribe);
    }
}
