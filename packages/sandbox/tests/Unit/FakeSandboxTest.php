...
describe('FakeSandbox', function () {
    ...
    it('streams stdout and stderr', function () {
        ...
        $chunks = [];
        $result = $sandbox->execute(['run'], null, function (string $type, string $chunk) use (&$chunks): void {
            $chunks[] = "{$type}:{$chunk}";
        });
    ...
    it('throws when no response is configured', function () {
        $sandbox = new FakeSandbox(ExecutionPolicy::in('/tmp'));
