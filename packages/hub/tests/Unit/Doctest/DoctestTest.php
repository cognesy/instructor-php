...
MARKDOWN;
            ...
            expect($doctest->markdownPath)->toBe('/test/doc.md');
            expect($doctest->code)->toContain('echo "Hello World"');
            expect($doctest->markdownTitle)->toBe('Test Document');
            expect($doctest->markdownDescription)->toBe('A test document for doctests');
            expect($doctest->caseDir)->toBe('examples/test');
            expect($doctest->casePrefix)->toBe('test_');
            expect($doctest->minLines)->toBe(1);
            expect($doctest->includedTypes)->toBe(['php']);
        });
    });
});