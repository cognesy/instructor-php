...
1. Start with `AgentLoop` ([Basic Agent](02-basic-agent)).
2. Move to `AgentBuilder` when composition becomes non-trivial ([AgentBuilder & Capabilities](13-agent-builder)).
3. Move to templates when config should be data-driven ([Agent Templates](14-agent-templates)).
4. Move to sessions when runtime state must persist between calls/processes ([Session Runtime](16-session-runtime)).
