...
`AgentState` is runtime state for a single agent loop. Persisted multi-request session concerns live in `AgentSession` / `SessionRuntime` (see [Session Runtime](16-session-runtime)).
