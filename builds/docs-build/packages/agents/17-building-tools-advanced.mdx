...
Most projects only need [Building Tools](06-building-tools) with `FunctionTool` or `BaseTool`. This page covers advanced patterns for when you need lower-level control: context-aware tools, raw `SimpleTool` subclasses, custom descriptors, the `ToolRegistry`, and deferred tool providers.
...
- [Tools](05-tools) -- overview, registration, contracts, and execution lifecycle
- [Building Tools](06-building-tools) -- quick path with `FunctionTool` and `BaseTool`
