...
This page covers the full tool API -- from creating and registering tools, through the contracts that govern them, to the execution lifecycle and error handling. For practical step-by-step guidance on building your own tools, see [Building Tools](06-building-tools).
...
For most projects, `FunctionTool` or `BaseTool` is all you need. See [Building Tools](06-building-tools) for practical guidance, and [Building Tools: Advanced Patterns](17-building-tools-advanced) for lower-level patterns.
...
- [Building Tools](06-building-tools) -- practical guide to creating tools with `FunctionTool` and `BaseTool`
- [Building Tools: Advanced Patterns](17-building-tools-advanced) -- `ContextAwareTool`, `SimpleTool`, descriptors, and schema strategies
- [Hooks](08-hooks) -- intercepting tool calls with `beforeToolUse` and `afterToolUse` hooks
