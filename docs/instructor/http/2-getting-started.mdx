...
description: 'Learn how to use the unified HTTP client API in your PHP project.'
---

## Installation

The unified HTTP client API is part of the Polyglot library, which provides a unified interface to various LLM providers. You can install it via Composer:

```bash
composer require cognesy/instructor-php
```

> NOTE: Currently unified HTTP client API is only available as a part of InstructorPHP package.
> In the future it will be available as a standalone package.


### Dependencies

The unified HTTP client API requires at least one of the supported HTTP client libraries. Depending on which client you want to use, you'll need to install the corresponding package:
...
Using the unified HTTP client API involves a few key steps:
...
The unified HTTP client API can be configured via configuration files or at runtime.
...
These examples demonstrate the basic usage of the unified HTTP client API for common HTTP operations. In the following chapters, we'll explore more advanced features and customization options.