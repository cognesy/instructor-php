...
Run `./bin/ins-hub list` you can see all the available tutorials and examples.
...
To read a tutorial, you can run `./bin/ins-hub show {id}` to see the full tutorial in the terminal.
...
To run a tutorial, you run `./bin/ins-hub run {id}` in terminal - it will execute the code and show the output. You need to have your OPENAI_API_KEY set in your environment (.env file in root directory of your copy of instructor-php repo).
...
This is mostly for testing if cookbooks are executed properly, but you can run `./bin/ins-hub all {id}` to run all the tutorials and examples in the terminal, starting from the one you specify.
