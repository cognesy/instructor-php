...
Put to make it easier you can just set the value in your `.env` file. `Settings` will pick it up automatically
...
Add required values to your .env:
```ini
# OpenAI API key
...
# Set up Instructor configuration path
INSTRUCTOR_CONFIG_PATH=<path/to/config>
```
