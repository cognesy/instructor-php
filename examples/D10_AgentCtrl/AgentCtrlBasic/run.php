...
if ($response->isSuccess()) {
    ...
    $answer = strtolower($response->text());
    assert(str_contains($answer, 'single responsibility'));
    assert((bool) preg_match('/open[\s\/-]*closed/', $answer));
    assert(str_contains($answer, 'liskov'));
    assert(str_contains($answer, 'interface segregation'));
    assert(str_contains($answer, 'dependency inversion'));
