        ...
        return StructuredOutput::using("openai")
            ->with(
                ...
                responseModel: Scalar::boolean(
                    "hasPII",
                    "True only if the text contains a phone number, SSN, credit card, or bank account number. False for age numbers, names, or job titles.",
                ),
            )
            ->getBoolean();
    }
...
$user = new StructuredOutput($runtime)
    ->with(messages: $text, responseModel: UserDetails::class)
    ->get();
