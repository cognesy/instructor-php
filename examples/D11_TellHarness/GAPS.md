...
| Test ergonomics | fyai documents an executable process-level operating model | `TellTestFactory` runs the real orchestration around a deterministic public driver; protocol tests execute a child process | Essential PHP and process-boundary testing are aligned; callers still own fixture state and cleanup | No essential gap for current automation |
